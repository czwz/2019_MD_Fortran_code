...
subroutine VERLET(xc,yc,zc,xp,yp,zp,v,T)
real*8,intent(inout)::  xc(:),yc(:),zc(:),xp(:),yp(:),zp(:),v(:)
real*8,allocatable::    fx(:),fy(:),fz(:),xf(:),yf(:),zf(:)
real*8::                r,factor,vx,vy,vz,v_factor
integer::               i,j,k,l
integer,intent(in)::    T
factor=eV*6.02*1000                                                         !anstromg/ps2
cutoff=2.5*ae                                                               !anstromg
        allocate(fx(atom),fy(atom),fz(atom),xf(atom),yf(atom),zf(atom))
        write(1,*) atom
        write(1,*) "  "
...
do i=1,atom
        ...
        do j=1,atom
                if (i.NE.j) then
                        ...
                        !write(*,*) i,j,rz
                        !if (i.EQ.1) then
                        !write(*,*) i,j,z(j),z(i),pbc(z(j)-z(i),ae),-(12*epsilon/sigma)*((sigma/r)**13-0.5*(sigma/r)**7)*(rz/r)
                        !endif
